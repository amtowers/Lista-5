...
```{r}
STF_Covid_19 %>% select(Assunto,Tempo_decisao) %>% drop_na() %>%
  mutate(Covid_19 = str_detect(Assunto,"COVID-19"),Covid_19 = case_when(Covid_19==TRUE~"Covid_19",Covid_19==FALSE~"Ind_Covid_19"))%>% ggplot(aes(fill=Covid_19,x=Tempo_decisao,color=Covid_19,y=Covid_19)) + stat_summary(fun=mean, geom="point") + stat_summary(fun.data=mean_ci, geom="errorbar", width=0.3) + geom_vline(data=. %>% group_by(Covid_19) %>% summarise(Tempo_decisao=mean(Tempo_decisao,na.rm = T)),
...
```{r}
STF_Covid_19 %>% select(Assunto,Tempo_decisao) %>% drop_na() %>%
  mutate(Covid_19 = str_detect(Assunto,"COVID-19"),Covid_19 = case_when(Covid_19==TRUE~"Covid_19",Covid_19==FALSE~"Ind_Covid_19")) %>% effectsize::cohens_d("Tempo_decisao","Covid_19",data=.)
```
